...
class Experiment
{
  public:
     ...
     * \param dataRate        Data rate for server
     * \param bAsync          If running async experiment, true
     * \param pflSymProvider  pointer to an fl-sim-interface (used to communicate with fl-sim)
     */
    Experiment(int numClients,
     ...
     * \return map of <client id, message>, messages to send back to fl-sim for each  client
     */
    // TODO: Change to run and change packet received
    std::map<int, FLSimProvider::Message> WeakNetwork(
        std::map<int, std::shared_ptr<ClientSession>>& packetsReceived,
        ns3::Time& timeOffset);
  ...
  private:
    ...
    std::string m_dataRate;    //!< Data rate for server
    bool m_bAsync;             //!< Indicator bool for whether experiment is async
    FLSimProvider* m_flSymProvider; //!< pointer to an fl-sim-interface (used to communicate)
