# flsim/network.py — ns-3 (terasim) aware adapter (STRICT: no fallbacks)
# - Wi-Fi/Ethernet: socket-only via wifi_exp binary, binary protocol
# - THz (6G): runs scratch app and parses a final JSON summary line (STRICT)
...
class Network:
    ...
    def sendRequest(self, requestType: int, array: Iterable[int]) -> Dict[int, Dict[str, float]]:
        ...
        if self._use_thz:
            items = self._run_thz(active_ids)              # STRICT
    ...
    def sendAsyncRequest(self, requestType: int, array):
        ...
        if self._use_thz:
            ...
            # optional timeout (seconds); default 0 → no deadline
            ts = float(self.thz_cfg.get("timeout_sec", 0))
            self._deadline = None if ts <= 0 else (time.time() + ts)
            return

        if not self._use_thz and self.controller != "socket":
            raise NotImplementedError("Wi-Fi/Ethernet in 'inline' mode is not implemented in this network.py. Use controller: 'socket'.")

        # --- Socket path (Wi-Fi/Ethernet): existing sender ---
        if self._sock is None:
            self.connect()
        if requestType != CMD_RUN_SIMULATION:
            raise ValueError(f"unsupported requestType={requestType}")
    ...
    def readAsyncResponse(self, timeout: float = 0.0):
        ...
        if self._use_thz:
            ...
            # finished: parse the last JSON line in the log (STRICT)
            if self._proc is not None:
                ...
                if not (getattr(self, "_thz_log_path", None) and self._thz_log_path.exists()):
                    raise RuntimeError("[thz] log file missing; ns-3 likely failed before printing JSON")
                ...
                with open(self._thz_log_path, "r") as fh:
                    for line in reversed(fh.read().splitlines()):
                        ...
                        if s.startswith("{") and s.endswith("}"):
                            ...
                            break
                if not last:
                    raise RuntimeError(f"[thz] ns-3 produced no JSON. Log: {self._thz_log_path}")

                data = json.loads(last)

                # Optional fixed window when ns-3 reports doneAt<=0 for a client
                sim_window = None
                if "sim_time" in self.thz_cfg:
                    try:
                        sim_window = float(self.thz_cfg["sim_time"])
                        if sim_window <= 0:
                            sim_window = None
                    except Exception:
                        sim_window = None
                ...
                for e in data.get("clientResults", []):
                    ...
                    if "doneAt" not in e or "rxBytes" not in e:
                        raise RuntimeError(f"[thz] malformed JSON for client {real}: {e}")
                    ...
                    if done <= 0.0:
                        if sim_window is None:
                            raise RuntimeError(f"[thz] client {real} has non-positive doneAt={done} and no sim_time set")
                        done = sim_window  # treat as time-boxed measurement
                ...
                if not q:
                    raise RuntimeError("[thz] ns-3 JSON contained no clientResults")
    ...
    def _run_thz(self, active_ids):
        """
        STRICT: run ns-3, parse the last JSON line, and return real results only.
        No fallback, no substitutions. If JSON is missing/invalid -> raise (and keep log).
        """
        import shlex, json, time, subprocess, os, tempfile, pathlib
        ...
        tmp = tempfile.NamedTemporaryFile(prefix="thz_", suffix=".log", delete=False)
        log_path = pathlib.Path(tmp.name)
        tmp.close()

        with open(log_path, "w", buffering=1) as fh:
            p = subprocess.Popen(args, cwd=str(self.NS3_ROOT), env=env,
                                stdout=fh, stderr=fh, text=True)

        # Optional deadline, ONLY if > 0
        ts = float(thz_cfg.get("timeout_sec", 0))
        deadline = (time.time() + ts) if ts > 0 else None
        ...
        # Read entire log; keep it on disk for debugging
        try:
            with open(log_path, "r") as fh:
                lines = fh.read().splitlines()
        except Exception as e:
            raise RuntimeError(f"[thz] could not read ns-3 log at {log_path}: {e}")
        ...
        if not last_json_line:
            rc = p.returncode
            raise RuntimeError(f"[thz] ns-3 produced no JSON (rc={rc}). Inspect log: {log_path}")
        ...
        for e in data.get("clientResults", []):
            ...
            real = int(id_map[local])
            if "doneAt" not in e or "rxBytes" not in e:
                raise RuntimeError(f"[thz] malformed JSON for client {real}: {e}. Log: {log_path}")
            done = float(e["doneAt"])
            rx   = float(e["rxBytes"])
            if done <= 0.0:
                # If you want to allow fixed-window goodput, set a positive sim_time in config.
                st = thz_cfg.get("sim_time", None)
                if st is None or float(st) <= 0.0:
                    raise RuntimeError(f"[thz] client {real} has non-positive doneAt={done} and no positive sim_time set. Log: {log_path}")
                done = float(st)
            items.append((real, done, rx / max(done, 1e-9)))

        if not items:
            raise RuntimeError(f"[thz] ns-3 JSON contained no usable clientResults. Log: {log_path}")
...
#                     # No fabrication. Fail loud so you can fix the run.
#                     raise RuntimeError(f"[thz] ns-3 produced no JSON. Log: {log_path}")

#                 data = json.loads(last_json_line)

#                 # Optional window for computing goodput if doneAt<=0 (time-boxed runs)
#                 sim_window = None
#                 if "sim_time" in self.thz_cfg:
#                     try:
#                         sim_window = float(self.thz_cfg["sim_time"])
...
#                 # Map local ids (0..len-1) back to real client ids
#                 id_map = {local: active_ids[local] for local in range(len(active_ids))}
#                 items = []
#                 for e in data.get("clientResults", []):
#                     local = int(e.get("id", -1))
...
#                     done = float(e.get("doneAt", 0.0) or 0.0)
#                     rx   = float(e.get("rxBytes", 0.0))

#                     if done <= 0.0:
#                         if sim_window is None:
#                             raise RuntimeError(f"[thz] client {real} has doneAt={done} and no sim_time set. JSON: {last_json_line}")
#                         done = sim_window  # goodput over time-boxed window

#                     thr = rx / done
#                     items.append((real, done, thr))

#                 if not items:
#                     raise RuntimeError(f"[thz] JSON had no clientResults. JSON: {last_json_line}")
...
#     def _run_thz(self, active_ids):
#         """
#         Run ns-3 THz scratch, write all output to a temp file, poll with a wall-clock
#         deadline, then parse the last JSON line. Never block on p.stdout.
#         """
#         import shlex, json, time, subprocess, os, tempfile, pathlib
...
#         # Write logs to a file to avoid pipe back-pressure & blocking reads
#         env = os.environ.copy()
#         env["NS_LOG"] = ""  # this won't mute NS_LOG_UNCOND, but it helps where it can
...
#         # Hard deadline (optional). Default 0 → disabled.
#         sim_time = float(thz_cfg.get("sim_time", 0.0))
#         timeout_sec = float(thz_cfg.get("timeout_sec", 0))
#         deadline = None if timeout_sec <= 0 else (time.time() + timeout_sec)
                
        

#         # Poll; don't read the pipe
#         while True:
#             rc = p.poll()
#             if rc is not None:
#                 break
...
#         if last_json_line is None:
#             sim_t = float(thz_cfg.get("sim_time", 0.8))
#             # No JSON: return "took sim_t, got 0 bytes" (throughput 0 but time > 0)
#             return [(int(active_ids[i]), float(sim_t), 0.0) for i in range(len(active_ids))]
...
#             done_raw = e.get("doneAt", e.get("roundTime", None))
#             rt = float(done_raw) if done_raw is not None else sim_t
#             if rt <= 0.0:
#                 rt = sim_t
...
#         if not items:
#             # Empty JSON list: still report sim_t so the round isn’t 0
#             return [(int(active_ids[i]), float(sim_t), 0.0) for i in range(len(active_ids))]
...
#         #     return [(int(active_ids[i]), 0.0, 0.0) for i in range(len(active_ids))]
